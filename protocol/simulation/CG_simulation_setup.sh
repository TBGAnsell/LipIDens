...
EOF
    ...
    if [[ -f system_ions_corrected.gro ]]; then echo "IMPORTANT: Please check you are happy with the protein orientation and bilayer position in system_ions_corrected.gro. If not the adjust the protein_shift and protein_rotate inputs and run again."; fi
    echo ""
...
EOF
    ...
    if [[ ! -f eq.1.gro ]]; then echo "Equilibration-1 failed!"; exit 0; fi
    ...
    if [[ ! -f eq.2.gro ]]; then echo "Equilibration-2 failed!"; exit 0; fi
